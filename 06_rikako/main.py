...
class Rikako(object):
    ...
    def open(self):
        ...
        with st.beta_expander("Vol.1 梅雨を楽しむ紫陽花", expanded=False):
            ...
            for n in range(1, 28):
                self.show_image(dir_path + str(n).zfill(2) + ".jpg", "jpg")
                if n == 2:
                    self.write_text("先日、奈良のお寺、「長弓寺」に早朝散歩に出掛けた。")
                    self.write_text("あいにくの雨。")
                    self.write_text("でも紫陽花を撮るなら悪くないなと、雨の日ならではの写真を撮ろうと意気込んで行った。")
                elif n == 4:
                    self.write_text("カタツムリを発見！")
                    self.write_text("超ミニサイズ。(寄りすぎて分からない)")
                    self.write_text("よくみるといっぱいいる。")
                elif n == 5:
                    self.write_text("カタツムリはマクロレンズで寄っても逃げないのでとても撮りやすい。")
                elif n == 6:
                    self.write_text("うなだれてしまった。")
                    self.write_text("")
                    self.write_text("")
                    self.write_text("かなりアクセスは悪いけど穴場スポット。")
                elif n == 7:
                    self.write_text("次は、同じ場所かと思いきや、こちらはかの有名な清水寺。")
                elif n == 8:
                    self.write_text("寄りで撮ったらどこで撮っても場所は関係ない。")
                    self.write_text("というわけで清水寺の紫陽花は以上。")
                elif n == 9:
                    self.write_text("続きましてこちらは京都の三室戸寺。")
                elif n == 10:
                    self.write_text("最近なんか花が小さい？ような気もするけど、だいぶ何年も撮りに出掛けた場所。")
                    self.write_text("(厳密にいうと花ではなくガク)")
                elif n == 11:
                    self.write_text("多種多様な紫陽花に圧倒される。")
                elif n == 12:
                    self.write_text("マクロレンズを手に入れるまではこれ以上寄るとピントが合わないのでこれが精一杯。")
                    self.write_text("でも壁紙には良いかも？と思って定番ショット化していた。")
                    self.write_text("が、なかなか画角一杯にまっすぐ入るような立派な紫陽花を見つけるのがまた難しい。")
                elif n == 13:
                    self.write_text("紫陽花を見た帰りに近くのカフェへ。")
                    self.write_text("炭酸水を入れると色が変わる、紫陽花ドリンク。")
                    self.write_text("")
                    self.write_text("さっきの写真となんとなくアプリで二重露出してみると…")
                elif n == 14:
                    self.write_text("あらなんとまあ爽やかなドリンク！")
                    self.write_text("二重露出編の集に目覚めたきっかけのショット。")
                elif n == 15:
                    self.write_text("一緒に食べたたまごサンドが美味でした…")
                    self.write_text("")
                    self.write_text("宇治の三室戸寺近くのカフェ。")
                    self.write_text("RAKU CAFE")
                    self.write_text("京都府宇治市宇治又振65 ")
                    self.write_text("")
                    self.write_text("女子受け間違いなし。")
                elif n == 17:
                    self.write_text("続きましては再び奈良へ。")
                elif n == 18:
                    self.write_text("ここ最近見つけた新たな紫陽花スポット。")
                    self.write_text("奈良の「矢田寺」。")
                elif n == 20:
                    self.write_text("大きな紫陽花が山一面に咲き誇っていて紫陽花のトンネルのよう。")
                elif n == 21:
                    self.write_text("紫陽花と着物。素敵。")
                elif n == 23:
                    self.write_text("風呂敷もまたよく合う。")
                    self.write_text("個人的意見ですが、服を持ち運ぶ時は基本風呂敷を使う派です。")
                    self.write_text("袋だと出し入れしにくい一方、風呂敷はさっと広げて綺麗に収納出来る上に、嵩張らない。おすすめです。")
                elif n == 25:
                    self.write_text("服に限らず、バッグや水筒代わりに。")
                elif n == 26:
                    self.write_text("そんなこんなで矢田寺は本当におすすめです。")
                elif n == 27:
                    self.write_text("以上紫陽花だけのコラムでした。")
                    self.write_text("")
                    self.write_text("梅雨の時期も紫陽花の時期ももうすぐ終わりますが、まだ間に合う！")
                    self.write_text("寄りで撮れば道端の紫陽花でも十分綺麗です。")
                    self.write_text("是非梅雨の花に目を向けてみてはいかがでしょうか。")
                    self.write_text("")
        ...
        with st.beta_expander("Vol.2 住むように過ごす 京都STAY 1", expanded=False):
            ...
            for n in range(1, 23):
                ...
                if n == 1:
                    ...
                    self.write_text("理香子", align="right")
        # Vol.3を表示
        with st.beta_expander("Vol.3 住むように過ごす 京都STAY 2", expanded=False):
        ...
        if self.is_released(dt.datetime(2021, 10, 2, 13, 00)):
            # Vol.4を表示
            with st.beta_expander("Vol.4 ほっとひと息 京都Cafe", expanded=True):
                dir_path = "06_rikako/vol.4/"
                for n in range(1, 26):
                    self.show_image(dir_path + str(n).zfill(2) + ".png", "png")
                    if n == 1:
                        st.markdown("""
                            今回のコラムは「京都のカフェ」

                            京都にはたくさん美味しいお店がありますが、今回はカフェに絞ってご紹介したいと思います。

                            今まで撮りまくった写真をまとめてみたらかなりの数のお店があったので、さくさく進んでいきます。
                            行きたいお店、要チェック！
                        """)
                    elif n == 2:
                        st.markdown("""
                        ...
                        st.markdown("""
                            ...
                            堀川今出川付近の落ち着いた珈琲専門店で、珈琲ゼリーがもう…本当に美味しい。
                        """)
                    elif n == 4:
                        st.markdown("""
                            「トリビュート コーヒー」

                            最近お気に入りしたカフェ。差し込む光が美しく、北欧インテリアに囲まれた静かで落ち着いた空間。
                        """)
                    elif n == 5:
                        st.markdown("""
                            「葦島珈琲」

                            三条河原町交差点付近のため、立地的にわりと混雑しがちなので早めの時間がおすすめ。
                        """)
                    elif n == 6:
                        st.markdown("""
                            皆さんご存知「ブルーボトルコーヒー」
                        ...
                        st.markdown("""
                            ...
                            絶対的おすすめ。四季の風情を中庭を眺めながら感じられる京都の大人空間。
                        """)
                    elif n == 8:
                        st.markdown("""
                            「サロン ド カンバヤシ」

                            こちらは髙見の結婚式会場だったアカガネリゾート内にあるカフェ。髙見よ、改めておめでとう！
                        """)
                    elif n == 9:
                        st.markdown("""
                            私の1番のお気に入り。静かで落ち着く私の隠れ家的空間なので流行りになってほしくないところ。
                            知りたい人は個人的にこそっと聞いてください。
                            """)
                    elif n == 10:
                        st.markdown("""
                            「喫茶フランソワ」

                            音楽関係者はほぼ行ってるであろうクラシック曲が流れている純喫茶。
                        """)
                    elif n == 11:
                        st.markdown("""
                            「ワイフ アンド ハズバンド」
                        ...
                        st.markdown("""
                            ...
                            鞄屋に併設されたカフェ。お店の奥にあり、庭園が眺められる。
                        """)
                    elif n == 13:
                        st.markdown("""
                            「イカリヤ コーヒー」

                            自家製粉米粉で作るスイーツと自家焙煎珈琲を楽しめる。
                        """)
                    elif n == 14:
                        st.markdown("""
                            「禅カフェ」

                            京都の老舗菓子店、鍵善良房が手がけるカフェ。大人空間で和菓子と珈琲を堪能できる。
                        """)
                    elif n == 15:
                        st.markdown("""
                            「カフェ&ギャラリー on」
                            禅がテーマの、白隠(はくいん)禅師のギャラリーの1階にあるカフェ。
                        """)
                    elif n == 16:
                        st.markdown("""
                            「クランプコーヒー サラサ」

                            京都にいくつかあるSARASAシリーズの自家焙煎珈琲のカフェ。
                        """)
                    elif n == 17:
                        st.markdown("""
                            「アカツキコーヒー」

                            一乗寺にあるコーヒー店。ちょこっとケーキなども共に味わえる。
                        """)
                    elif n == 18:
                        st.markdown("""
                            「キルン」

                            1階はコーヒーショップ。2階にはレストランが併設されている。
                        """)
                    elif n == 19:
                        st.markdown("""
                            「前田珈琲」

                            明倫店は旧小学校にあり、レトロな空間で楽しめる珈琲は味わい深い。
                        """)
                    elif n == 20:
                        st.markdown("""
                            「火裏蓮花」

                            細い路地裏の奥にあるお店。カレーも美味しい。
                        """)
                    elif n == 21:
                        st.markdown("""
                            「ソングバード コーヒー」

                            オリジナルブレンドコーヒーが味わえるが、鳥の巣カレーとたまごサンドがとても美味しい。
                        """)
                    elif n == 22:
                        st.markdown("""
                            「珈琲工房てらまち」
                        ...
                        st.markdown("""
                            ...
                            烏丸御池から烏丸通りを少し下ったところにあるTHEイギリスなカフェ。バノフィーパイがとても美味しい。
                        """)
                    elif n == 25:
                        st.markdown("""
                            今回はここまで。珈琲が多めでしたが次回はケーキ、軽食系もご紹介。
